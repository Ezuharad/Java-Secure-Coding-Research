...
public class Client extends SimpleClient
{
    ...
    private Message.Request parseLine(String line)
    {
        ...
        if (!iter.hasNext())
            return null;
        String msgType = iter.next().toUpperCase();  // Violates STR02
        if (msgType.equals("STARTUP"))
        {
            ...
            while (iter.hasNext())
            {
               ...
               if (next.toLowerCase().equals("snappy"))  // Violates STR02
               {
        ...
        {
            line = line.substring(6);  // Violates EXP01
            // Ugly hack to allow setting a page size, but that's playground code anyway
            String query = line;
            int pageSize = -1;
            if (line.matches(".+ !\\d+$"))
            {
                int idx = line.lastIndexOf('!');  // Violates EXP01
                query = line.substring(0, idx-1);  // Violates EXP01
                try
                {
                    pageSize = Integer.parseInt(line.substring(idx+1, line.length()));  // Violates NUM00
                }
        ...
        {
            String query = line.substring(8);  // Violates EXP01
        ...
        else if (msgType.equals("EXECUTE"))
        {
            ...
            catch (Exception e)  // Very bad practice
            {
                return null;
            }
        }
        ...
        {
            String type = line.substring(9).toUpperCase();  // Violates EXP01
            try
            {
    ...
    private Map<String, String> readCredentials(Iterator<String> iter)
    {
        ...
        return credentials;  // Not technically a violationm of OBJ04
    }
    ...
    private byte[] encodeCredentialsForSasl(Map<String, String> credentials)
    {
        ...
        byte[] initialResponse = new byte[username.length + password.length + 2];  // Violates NUM00, EXP01
        initialResponse[0] = 0;
        System.arraycopy(username, 0, initialResponse, 1, username.length);
        initialResponse[username.length + 1] = 0;  // Violates EXP01, NUM00
        System.arraycopy(password, 0, initialResponse, username.length + 2, password.length);  // Violates EXP01, NUM00
        return initialResponse;
    }

    public static void main(String[] args) throws Exception  // Very bad practice
    {
