...
{
    public static CassandraVersion SUPPORTS_GLOBAL_PREPARE_FLAG_VERSION = new CassandraVersion("2.2.1");  // Violates OBJ10
    ...
    {
        this.failureDetector = failureDetector;  // Violates EXP01
        this.gossiper = gossiper;  // Violates EXP01
    }
     ...
     */
    public static Set<InetAddress> getNeighbors(String keyspaceName, Range<Token> toRepair, Collection<String> dataCenters, Collection<String> hosts)  // Violates EXP01
    {
    ...
    public synchronized UUID prepareForRepair(UUID parentRepairSession, Set<InetAddress> endpoints, RepairOption options, List<ColumnFamilyStore> columnFamilyStores)  // Violates EXP01
    {
    ...
    public Set<SSTableReader> currentlyRepairing(UUID cfId, UUID parentRepairSession)  // Violates EXP01
    {
        ...
        for (Map.Entry<UUID, ParentRepairSession> entry : parentRepairSessions.entrySet())
        {
            ...
            if (sstables != null && !entry.getKey().equals(parentRepairSession))
                repairing.addAll(sstables);  // Violates EXP00
        }
        return repairing;
    }
     ...
     */
    public synchronized ListenableFuture finishParentSession(UUID parentSession, Set<InetAddress> neighbors, Collection<Range<Token>> successfulRanges)  // Violates EXP01
    {
        List<ListenableFuture<?>> tasks = new ArrayList<>(neighbors.size() + 1);  // Violates NUM00
    ...
    public ListenableFuture<List<Object>> doAntiCompaction(final UUID parentRepairSession, Collection<Range<Token>> successfulRanges)
    {
        ...
        if (!successfulRanges.isEmpty())  // Violates EXP01
        {
    ...
    {
        RepairJobDesc desc = message.desc;  // Violates EXP01
        RepairSession session = sessions.get(desc.sessionId);
        if (session == null)
            return;
        switch (message.messageType)  // Violates EXP01
        {
            case VALIDATION_COMPLETE:
                ValidationComplete validation = (ValidationComplete) message;  // Should probably check if instance of
                session.validationComplete(desc, endpoint, validation.trees);
                break;
            case SYNC_COMPLETE:
                // one of replica is synced.
                SyncComplete sync = (SyncComplete) message;  // Should probably check if instance of
    ...
    public static class ParentRepairSession  // Violatees OBJ08
    {
        ...
        {
            for (ColumnFamilyStore cfs : columnFamilyStores)  // Violates EXP01
