...
public final class Batch
{
    ...
    public int size()
    {
        return decodedMutations.size() + encodedMutations.size();  // Violates NUM00
    }
    ...
    static final class Serializer implements IVersionedSerializer<Batch>
    {
        ...
        {
            assert batch.encodedMutations.isEmpty() : "attempted to serialize a 'remote' batch";  // Ignored as an assert

            long size = UUIDSerializer.serializer.serializedSize(batch.id, version);  // Violates EXP01
            size += sizeof(batch.creationTime);  // Violates NUM00, EXP01

            size += sizeofUnsignedVInt(batch.decodedMutations.size());  // Violates NUM00, EXP01
            for (Mutation mutation : batch.decodedMutations)  // Violates EXP01
            {
                int mutationSize = (int) Mutation.serializer.serializedSize(mutation, version);
                size += sizeofUnsignedVInt(mutationSize);  // Violates NUM00
                size += mutationSize;  // Violates NUM00
            }
        ...
        public void serialize(Batch batch, DataOutputPlus out, int version) throws IOException
        {
            ...
            UUIDSerializer.serializer.serialize(batch.id, out, version);  // Violates EXP01
            out.writeLong(batch.creationTime);  // Violates EXP01

            out.writeUnsignedVInt(batch.decodedMutations.size());  // Violates EXP01
            for (Mutation mutation : batch.decodedMutations)  // Violates EXP01
            {
                out.writeUnsignedVInt(Mutation.serializer.serializedSize(mutation, version));  // Violates EXP01
        ...
        public Batch deserialize(DataInputPlus in, int version) throws IOException
        {
            ...
            long creationTime = in.readLong();  // Violates EXP01
        ...
        {
            int count = (int) in.readUnsignedVInt();  // Violates EXP01

            ArrayList<ByteBuffer> mutations = new ArrayList<>(count);
            for (int i = 0; i < count; i++)  // Does not check if i < 0?
        ...
        private static Collection<Mutation> decodeMutations(DataInputPlus in, int version) throws IOException
        {
            ...
            for (int i = 0; i < count; i++)  // Does not check if i < 0?
            {
                in.readUnsignedVInt(); // skip mutation size  // Violates EXP01
