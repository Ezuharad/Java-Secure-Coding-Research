...
public abstract class Cell extends ColumnData
{
    ...
    {
        int cmp = c1.column().compareTo(c2.column());  // Violates EXP01
        if (cmp != 0)
            return cmp;

        Comparator<CellPath> pathComparator = c1.column().cellPathComparator(); // Violates EXP01
        return pathComparator == null ? 0 : pathComparator.compare(c1.path(), c2.path());  // Violates EXP01
    };
